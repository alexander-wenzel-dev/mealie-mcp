...
```json
{
  "mcpServers": {
    "mealie": {
      "command": "uvx",
      "args": ["--from", "git+https://github.com/alexander-wenzel-dev/mealie-mcp.git@v0.3.0", "mealie-mcp"],
      "env": {
...
```sh
claude mcp add mealie --env MEALIE_BASE_URL=https://mealie.example.com --env MEALIE_API_TOKEN=replace-me -- uvx --from git+https://github.com/alexander-wenzel-dev/mealie-mcp.git@v0.3.0 mealie-mcp
```
...
| `recipes_units`               | Create, read, list, update, delete, and merge ingredient units.                     |
