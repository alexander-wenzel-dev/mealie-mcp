...
Bumping the pinned spec to a new Mealie version reads that version from `$MEALIE_BASE_URL`: both `regen-client --update` and the live confirmation that the new version behaves as the task expects fetch from it. The instance the environment starts with runs the currently pinned version, so it being older than the bump target is the expected starting state, not a blocker. Make the target version reachable first, then proceed. (autonomous) The CI service container holds port 9000 on the old version, so boot the target alongside it with `scripts/mealie-up --version <tag> --port 9001`, point `MEALIE_BASE_URL` at `http://localhost:9001`, and `docker rm -f mealie-mcp-dev` at the end; the do-not-bootstrap rule of the autonomous live path does not cover obtaining a bump target. (interactive) With `.env` present the operator owns the instance, so ask them to point it at the target version rather than bootstrapping over their target; without `.env`, boot the target with `scripts/mealie-up --version <tag>` as in the live-test bootstrap path.

The generator does not runtime-validate response bodies. Its `_parse_response` functions use `cast(T, response.json())`, which is a type hint with no runtime effect. Keep explicit `isinstance` shape guards on parsed responses at the tool boundary. The same `cast` fallback sits in the model parsers on the request side: a union arm that fails leaves the caller's raw list on the model and breaks in `to_dict`, so parse caller-supplied list items against their own model before building a body. Model defaults and field types describe the spec, not Mealie's runtime behaviour; confirm write semantics against a live call before reasoning from the client alone. `openapi-python-client` is pinned via `[tool.uv.sources]` to a fork branch carrying two unmerged generator fixes (upstream PRs openapi-generators/openapi-python-client#1448 and #1449); without that pin the generator silently drops dashed-schema models (`Recipe-Input`, `Recipe-Output`, `RecipeIngredient-*`, `RecipeCommentOut-*`, `ShoppingList*`) and every endpoint that references them, including the recipe PUT and PATCH and the shopping-list write endpoints. Drop the pin and bump the floor when a release containing both lands. If the generator drops models the pin does not cover, surface it as a blocker rather than hand-writing replacements.
